...
let removeOldStylesheets = ( project_directory ) => {
	...
	return new Promise( ( resolve, reject ) => {
		let boilerplate_scss_files = shell.find(`${project_directory}/${project_directories.stylesheet}`).filter(function(file) { return file.match(/\.scss$/); });
		if ( !boilerplate_scss_files.length ) reject(messages.create.no_boilerplate_scss_files);
		shell.rm(boilerplate_scss_files);
		resolve();
/*		shell.exec( `find ./${project_directories.stylesheet} -maxdepth 1 -type f -exec rm {} \\;`, { cwd: project_directory, silent: true }, ( err ) => {
			if ( err ) {
				console.log( err );
				reject();
			}
...
/*		shell.cp(`${project_directory}/${project_directories.stylesheet}/dist/${preprocessor}/!*`, `${project_directory}/${project_directories.stylesheet}`);
		shell.find(`${project_directory}/${project_directories.stylesheet}`)*/

		shell.exec( `cp ./${project_directories.stylesheet}/dist/${preprocessor}/* ./${project_directories.stylesheet}/ && find ./${project_directories.stylesheet} -maxdepth 1 -type f -exec rename 's/\.dist$//' {} \\;`,
