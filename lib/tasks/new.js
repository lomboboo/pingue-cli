...
let removeOldStylesheets = ( project_directory ) => {
	...
	return new Promise( ( resolve, reject ) => {
		let boilerplate_scss_files = shell
			.find(`${project_directory}/${project_directories.stylesheet}`)
			.filter((file)=>{return file.match(/\.scss$/)});
...
let generateStylesheets = ( project_directory, preprocessor ) => {
	return new Promise( ( resolve, reject ) => {
		...
		shell.exec( `cp ${config.ROOT}/${project_directories.dist.blueprints.new}/${preprocessor}/* ./${project_directories.stylesheet}/`,
